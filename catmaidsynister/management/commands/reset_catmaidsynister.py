...
class Command(BaseCommand):
    help = 'Deletes all rows from database tables backing the catmaidsynister models.'
    ...
    def handle(self, *args, **options):
        ...
        while selection.lower() not in ['y', 'n', '']:
            selection = input('This will empty all catmaidsynister-related database tables. Are you sure ([y]/n)? ')
        ...
        for model in apps.get_app_config('catmaidsynister').get_models():
        ...
        self.stdout.write(self.style.SUCCESS('Successfully cleared catmaidsynister tables'))
